...
class OrderViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
        ...
        databaseRef.observe(DataEventType.value, with: {(DataSnapshot) in
            if DataSnapshot.childrenCount > 0 {
                ...
                for customers in DataSnapshot.children.allObjects as! [DataSnapshot]{
                    ...
                    let customerMotion = customerObject?["OptionMotionShow1"]
                    let customerTwist = customerObject?["OptionTwistShow1"]
                    let customerAmount = customerObject?["OptionAmountShow1"]
                    ...
                    let customer = Customer(name: (customerName as! String?)!, time: (customerTime as! String?)!, itemName: (customerItemname as! String?)!, itemType: (customerItemtype as! String?)!, motion: (customerMotion as! String?)!, twist: (customerTwist as! String?)!, amount: (customerAmount as! String?)!, qty: (customerQty as! String?)!, image: (customerImage as! String?)!, times: (customerTimes as! String?)!, date: (customerDate as! String?)!, price: (customerPrice as! String?)!)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailLbl!.text = "Order list \(customer.itemName) \(customer.itemType) \(customer.motion) \(customer.twist) \(customer.amount) \(customer.qty)"
