...
class HistoryViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
        ...
        databaseRef.observe(DataEventType.value, with: {(DataSnapshot) in
            if DataSnapshot.childrenCount > 0 {
                ...
                for historys in DataSnapshot.children.allObjects as! [DataSnapshot]{
                    ...
                    let historyMotion = historyObject?["OptionMotionShow1"]
                    let historyTwist = historyObject?["OptionTwistShow1"]
                    let historyAmount = historyObject?["OptionAmountShow1"]
                
                    let history = History(cusName: (customerName as! String?)!, date: (historyDate as! String?)!, name: (historyName as! String?)!, type: (historyType as! String?)!, motion: (historyMotion as! String?)!, twist: (historyTwist as! String?)!, amount: (historyAmount as! String?)!, qty: (historyQty as! String?)!, time: (historytime as! String?)!)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.detailLbl.text = "Order list:  " + history.name + " " + history.type + " " + history.motion + " " + history.twist + " " + history.amount + " " + history.qty
        return cell
    }
