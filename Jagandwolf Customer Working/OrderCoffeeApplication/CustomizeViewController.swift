...
class CustomizeViewController: UIViewController {
    ...
    var price = Double()
    var priceSelect = Double()
    var priceNeat = Double()
    var priceChilled = Double()
    var priceRocks = Double()
    ...
    override func viewDidLoad() {
        ...
        databaseRef.child("Product").child(getShopId).child(getItemName).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            let motion = value?["OptionMotion"] as? String ?? ""
            let twist = value?["OptionTwist"] as? String ?? ""
            let amount = value?["OptionAmount"] as? String ?? ""
    ...
    @IBAction func addOrderBtn(_ sender: Any) {
        if itemType == "" || motion == "" || twist == "" || amount == "" {
            ...
            self.save(name: getItemName, type: itemType, qty: "\(qty)", motion: motion, twist: twist, amount: amount, price: "\(price)")
            self.performSegue(withIdentifier: "goToList", sender: self)
        }
    }
    
    func save(name: String, type: String, qty: String, motion: String, twist: String, amount: String, price: String) {
        ...
        order.setValue(motion, forKey: "motion")
        order.setValue(twist, forKey: "twist")
        order.setValue(amount, forKey: "amount")
